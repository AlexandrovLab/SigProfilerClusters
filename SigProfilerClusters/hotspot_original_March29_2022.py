...
def first_run (distances, distances_orig_all, distances_orig, vcf_path_clust, vcf_path_nonClust, sample, original, sim_count, project, genome, clustering_vaf, correctionData=False, correction=False, regions=None, imds_corrected=None, windowSize=None, chromLengths=None):
	...
	# 	if correction:
	# 		if len(regions) > 0:
	# 			clustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if int(x[0]) <= distance_cut or (regions[catch(x, regions, chromLengths, genome)] - (int(x[3]) + chromLengths[genome][x[2]]) < windowSize and cutoffCatch(x, imds_corrected, regions, catch(x, regions, chromLengths, genome), distance_cut, chromLengths[genome], windowSize))]# int(x[0]) < imds_corrected[x[1]][regions[catch(x, regions, chromLengths, genome, imds_corrected[x[1]])]]))]
	# 			nonClustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if x[1:] + [x[0]] not in clustered_muts]
	# 		else:
	# 			clustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if int(x[0]) <= distance_cut ]
	# 			nonClustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if x[1:] + [x[0]] not in clustered_muts]	
	# 	else:
	# 		clustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if int(x[0]) <= distance_cut]
	# 		nonClustered_muts = [x[1:] + [x[0], y] for x,y in zip(distances_orig_all, imdsProbabilities) if x[1:] + [x[0]] not in clustered_muts]
	# else:
	# 	clustered_muts = [x[1:] + [y] for x,y in zip(distances_orig_all, imdsProbabilities) if int(x[0]) <= distance_cut]
	# 	nonClustered_muts = [x[1:] + [y] for x,y in zip(distances_orig_all, imdsProbabilities) if int(x[0]) > distance_cut]
	...
	# 			if clustering_vaf:
	# 				vaf = muts[5]
	# 				print("\t".join([project,sample,".",genome,"SNP",chrom,pos,pos,ref,alt,"SOMATIC",vaf, plotIMD, imd_recorded, imdProbs]),file=clust)
	# 			else:
	# 				print("\t".join([project,sample,".",genome,"SNP",chrom,pos,pos,ref,alt,"SOMATIC", plotIMD, imd_recorded, imdProbs]),file=clust)
	...
	# 			imd_recorded = muts[-2]
	# 			imdProbs = str(muts[-1])
	# 			print("\t".join([project,sample,".",genome,"SNP",chrom,pos,pos,ref,alt,"SOMATIC", plotIMD, imd_recorded, imdProbs]),file=nonclust)
	# distance_cut = 1000
	if not correctionData:
		if correction:
			if len(regions) > 0:
...
def densityCorrection (densityMuts, densityMutsSim, windowSize):
	...
	'''
	# combinedSimDensity = [ele for ele in densityMutsSim.values()]
	# combinedSimDensity = list(chain.from_iterable(combinedSimDensity))

	# x,y = sns.kdeplot(np.log10(combinedSimDensity), bw_adjust=20, gridsize=200).get_lines()[0].get_data()
	# plt.close()
	# densityMuts = np.log10(densityMuts)
	# imdsProbabilities = []
	# for dist in densityMuts:
	# 	imdsProbabilities.append(y[bisect.bisect_left(x, dist)])
