...
def plot_clustered (orig_bins, sim_bins, bincenters2, sample, panel4):
	...
	#panel4.legend([sim, orig], ['simulated', 'real samples'])


	#plt.title(sample)
	
	#pp_clustered.savefig()
	#plt.close()
	
def plot_non_clustered (orig_bins, sim_bins, bincenters2, sample, panel6):
	...
	panel6.set_xlabel("Distance", fontweight ='bold', fontsize=15)
	#axes.set_ylabel("Count")
	# panel6.set_xlim(1,)
	# panel6.set_ylim(1,)

	#panel6.legend([sim, orig], ['simulated', 'real samples'])


	#plt.title(sample)
	
	#pp_non_clustered.savefig()
	#plt.close()



def plot_hist (distances, distances_orig, original_vcf, vcf_path_clust, vcf_path_nonClust, sample, pp, original, sim_count, matrix_path_clustered, matrix_path_nonClustered, panel2, panel3, panel4, panel5, panel6):
	...
	for i in range (0, len(total_bin_counts[0]), 1):
		...
		if count/sim_count < 1:
		#if count/1 < 1:
			avg_bin_counts.append(0)
		else:
			avg_bin_counts.append(count/sim_count)
			#avg_bin_counts.append(count/1)
		# ci_lower.append(mean_confidence_interval(bin_std_dev)[2])
		# ci_upper.append(mean_confidence_interval(bin_std_dev)[3])
		# std_dev.append(statistics.stdev(bin_std_dev))
	...
	#for i in range(0, len(ci_lower), 1):
	#	print(ci_lower[i], ci_upper[i], bincenters2[i], avg_bin_counts[i])
	...
	# conf_higher = plt.fill_between(bincenters2, ci_lower, ci_upper, alpha=0.5)
	...
	if original:
		...
		panel3.text(0.08, 0.575, "clustered=" + str(orig_mutations) + "(" + str(round(interval_percent, 2)) + "%)",transform=plt.gcf().transFigure)
	...
	if orig_mutations > 50 and p < 0.01:
		plot_clustered (y2[:interval_line+1], avg_bin_counts[:interval_line+1], bincenters2, sample, panel4)
		plot_non_clustered (y2[interval_line+1:], avg_bin_counts[interval_line+1:], bincenters2, sample, panel6)
		return(True)
	else:
		return (False)

	# distance_cut = bincenters2[interval_line]
	# with open (original_vcf) as f, open(vcf_path_clust + "BRCA_clustered.txt", 'w') as clust, open(vcf_path_nonClust + "BRCA_nonClustered.txt", 'w') as non:
	# 	initial_line = True
	# 	for lines in f:
	...
	# 						print('\t'.join([x for x in line]), file=clust)
	# 					else:
	# 						print('\t'.join([x for x in line]), file=non)
	# 		init_start = start
	# os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p BRCA_clustered -g GRCh37")
	# os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p BRCA_nonClustered -g GRCh37")

	# if not os.path.exists(matrix_path_clustered):
	# 	os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p BRCA_clustered -g GRCh37")
	# if not os.path.exists(matrix_path_nonClustered):
	# 	os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p BRCA_nonClustered -g GRCh37")


	# panel2.grid(which='major', axis='y', color=[0.93,0.93,0.93], zorder=1)
	# panel4.grid(which='major', axis='y', color=[0.93,0.93,0.93], zorder=1)
	# panel6.grid(which='major', axis='y', color=[0.93,0.93,0.93], zorder=1)




#############################################################################################
# Plot 96 bar plot (all mutations)
#############################################################################################
...
def plot96_same (matrix_path, matrix_path_clustered, matrix_path_nonClustered, sample, percentage, signature, panel1, panel3, panel5, fig):
	...
	for i in range (0, 96, 1):
		plt.text(i/177 + .075, .052, xlabels[i][0], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		...
		plt.text(i/177 + .075, .068, xlabels[i][2], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		count += 1
		if count == 16:
			count = 0
			m += 1	
	...
	# if signature:
	# 	panel1.set_ylabel("Mutation Percentage", fontsize=10, fontname="Times New Roman", weight = 'bold')
	# else:
	# 	panel1.set_ylabel("Mutation Counts", fontsize=10, fontname="Times New Roman", weight = 'bold')
	...
	m = 0
	for i in range (0, 96, 1):
		plt.text(i/177 + .075, .3495, xlabels[i][0], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		...
		plt.text(i/177 + .075, .3655, xlabels[i][2], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		count += 1
		if count == 16:
			count = 0
			m += 1
	...
	m = 0
	for i in range (0, 96, 1):
		plt.text(i/177 + .075, .647, xlabels[i][0], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		...
		plt.text(i/177 + .075, .663, xlabels[i][2], fontsize=6, color='gray', rotation='vertical', verticalalignment='center', fontname='Courier New', transform=plt.gcf().transFigure)
		count += 1
		if count == 16:
			count = 0
			m += 1
...
def plotINDEL_same (matrix_path, matrix_path_clustered, matrix_path_nonClustered, sample, percentage, signature, panel1, panel3, panel5, fig):
	...
	x += .02
	plt.text(x, yText_labels_bottom, '1  2  3  4  5+', fontsize=5.7, fontweight='bold', fontname='Times New Roman', color='black', transform=plt.gcf().transFigure)
...
def first_run (distances, distances_orig, original_vcf, vcf_path_clust, vcf_path_nonClust, sample, original, sim_count, matrix_path_clustered, matrix_path_nonClustered):
	maximum_sim = 0
	try:
		maximum_orig = max(distances_orig)
	except:
	...
	if sample == 'PD10010a':
		out = open ("PD10010a_bins.txt", 'w')
		print("\t".join(str(x) for x in bin1), file = out)

		for binned in total_bin_counts:
			if sample == 'PD10010a':
				#print(binned, file=out)
				print("\t".join(str(x) for x in binned), file=out)

	if sample == 'PD13425a':
		out2 = open ("PD13425a_bins.txt", 'w')
		print("\t".join(str(x) for x in bin1), file = out2)

		for binned in total_bin_counts:
			if sample == 'PD13425a':
				#print(binned, file=out)
				print("\t".join(str(x) for x in binned), file=out2)
	...
	if orig_mutations > 50 and p < 0.01:
		...
		with open (original_vcf) as f, open(vcf_path_clust + "BRCA_clustered.txt", 'a') as clust, open(vcf_path_nonClust + "BRCA_nonClustered.txt", 'a') as non:
			initial_line = True
...
def main ():
	...
	original_vcf = args.vcf
	#original_vcf = ref_dir + "/references/vcf_files/" + project + "/SNV/breast_genomes_all_subs_simple_1.txt"
	directory = ref_dir + '/simulation_output/' + project + "_intradistance_" + genome + "_" + file_context + "/"
	directory_out = ref_dir + "/simulation_output/" + project + "_simulations_" + genome + "_" + file_context + "_intradistance_plots/"
	#directory_orig = ref_dir + "/simulation_output/" + project + "_intradistance_original_" + genome + "_" + file_context + "/"
	directory_orig = ref_dir + "/simulation_output/BRCA_intradistance_original_" + genome + "_" + file_context + "/"

	#original_vcf = os.listdir(ref_dir + "/references/vcf_files/" + project + "/SNV/")
	if file_context == '96':
		vcf_path_clust = ref_dir + "/references/vcf_files/" + project + "_clustered/SNV/"
		vcf_path_nonClust = ref_dir + "/references/vcf_files/" + project + "_nonClustered/SNV/"
	else:
	...
	# matrix_path = "/Users/ebergstr/Desktop/Perl_tests/testCode/simulation_code_python/mutation_simulation/references/matrix/BRCA/BRCA.SBS96.all"
	# matrix_path_clustered = "/Users/ebergstr/Desktop/Perl_tests/testCode/simulation_code_python/mutation_simulation/references/matrix/BRCA_clustered/BRCA_clustered.SBS96.all"
	# matrix_path_nonClustered = "/Users/ebergstr/Desktop/Perl_tests/testCode/simulation_code_python/mutation_simulation/references/matrix/BRCA_nonClustered/BRCA_nonClustered.SBS96.all"
	# output_path = directory_out + project + '_intradistance_plots_' + file_context + '.pdf'

	matrix_path = ref_dir + "/references/matrix/" + project + "/" + project + matrix_file_suffix + "all"
	matrix_path_clustered = ref_dir + "/references/matrix/" + project + "_clustered/" + project + "_clustered" + matrix_file_suffix + "all"
	matrix_path_nonClustered = ref_dir + "/references/matrix/" + project + "_nonClustered/" + project + "_nonClustered" + matrix_file_suffix + "all"
	output_path = directory_out + project + '_intradistance_plots_' + file_context + '.pdf'
		...
		for folder in folders:
			...
			if original:
				if first_sort:
					...
					output.close()
					first_sort = False
				with open (directory_orig + sample + "_" + file_context + "_intradistance.txt") as f2:
					for lines in f2:
						line = lines.strip()
						if int(line) >= 1:
			...
			first_run(overall_distances, distances_orig, original_vcf, vcf_path_clust, vcf_path_nonClust, sample, original, sim_count, matrix_path_clustered, matrix_path_nonClustered)
			print(sample)
		if file_context == 'INDEL':
			os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p " + project + "_clustered -g " + genome + " -i")
			os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p " + project + "_nonClustered -g " + genome + " -i")
		else:
			os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p " + project + "_clustered -g " + genome)
			os.system("python3 sigProfilerMatrixGenerator_withBED_SNV_INDEL_together_faster_fdr.py -p "+ project + "_nonClustered -g " + genome)



	pp = PdfPages(directory_out + project + '_intradistance_plots_' + file_context + '.pdf')
	...
	for folder in folders:
		...
		# if l == 1:
		# 	break
		clustered = plot_hist(overall_distances, distances_orig, original_vcf, vcf_path_clust, vcf_path_nonClust, sample, pp, original, sim_count, matrix_path_clustered, matrix_path_nonClustered, panel2, panel3, panel4, panel5, panel6)
		if clustered:
			if file_context == '96':
