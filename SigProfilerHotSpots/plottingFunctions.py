...
def rainfall (chrom_based_IMD, project, project_path, chrom_path, windowSize = 10000000):
	aggregated = False
	chrom_based = False
	...
	projectPath = project_path + "output/vcf_files/ESCC_clustered/subclasses" + path_suffix + "/"
	...
	classes = ["Class IA", "Class IB", "Class IC", "Class II", "Class III", "Non-clust"]
	mutationsPath = [projectPath + "class1a/ESCC_clustered_class1a.txt", projectPath + "class1b/ESCC_clustered_class1b.txt", projectPath + "class1c/ESCC_clustered_class1c.txt",
					projectPath + "class2/ESCC_clustered_class2.txt", projectPath + "class3/ESCC_clustered_class3.txt", projectPath_parent + "output/vcf_files/ESCC_nonClustered/SNV/ESCC_nonClustered.txt"]
	...
	headerFields = ["project", "samples","ID","genome","mutType","chr","start","end", "ref", "alt", "mutClass", "vaf","class", "failedReason"]
	...
	if classes[0] == "Class III":
		mutations = pd.read_csv(mutationsPath[0], sep="\t", names=headerFields, header=None)
	elif classes[0] == "Non-clust":
		mutations = pd.read_csv(mutationsPath[0], sep="\t", names=headerFields[:-3], header=None, skiprows=[0])
		...
		mutations = pd.read_csv(mutationsPath[0], sep="\t", names=headerFields[:-1], header=None)
	...
	if len(mutationsPath) > 1:
		for i in range(1, len(mutationsPath), 1):
			if classes[i] == "Class III":
				newMutations = pd.read_csv(mutationsPath[i], sep="\t", names=headerFields, header=None)
			elif classes[i] == "Non-clust":
				newMutations = pd.read_csv(mutationsPath[i], sep="\t", names=headerFields[:-3], header=None, skiprows=[0])
			else:
				newMutations = pd.read_csv(mutationsPath[i], sep="\t", names=headerFields[:-1], header=None)
	...
	colors = ['red', 'black', 'green', 'orange', 'blue', 'grey']
	...
	if not chrom_based:
						...
						try:
							...
							if not aggregated:
								if subclass == "Class III":
									if chrom_based_IMD:
										try:
											plotIMDs = [[x,y] for x,y in zip(starts, minIMDs) if y <= imdsData[sample.split("_")[0]][chrom]]
										except:
											plotIMDs = [[x,y] for x,y in zip(starts, minIMDs) if y <= imdsDataSample[sample.split("_")[0]]]
									else:
										plotIMDs = [[x,y] for x,y in zip(starts, minIMDs) if y <= imdsData[sample.split("_")[0]]]
								else:
		...
		with open(projectPath_parent + "output/simulations/data/originalCounts" + path_suffix + ".pickle", "wb") as f:
			pickle.dump(totalMutsIMD, f)
		with open(projectPath_parent + "output/simulations/data/originalCounts1000" + path_suffix + ".pickle", "wb") as f:
